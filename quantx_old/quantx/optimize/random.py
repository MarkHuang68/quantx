...
class RandomOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        self.logger.info(f"[random] 開始對 {self.strategy_cls.__name__} ({symbols_str}) 執行 {self.n_trials} 次隨機嘗試...")
        ...
        for i in range(self.n_trials):
            ...
            try:
                ...
                self.logger.warning(f"[trial {i+1}/{self.n_trials}] failed with params={params}: {e}", exc_info=True)
                continue
            ...
            self.logger.info(
                f"[trial {i+1}/{self.n_trials}] sharpe={rec.get('sharpe_ratio', 0):.4f} "
